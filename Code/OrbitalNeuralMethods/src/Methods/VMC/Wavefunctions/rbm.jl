...
function evaluate(particles, wf::RBM)::Float64
...
function kinetic(particles, wf::RBM)::Float64
    X = particles.positions
    dder = 0.0
    for m in 1:wf.M
        sum1 = 0.0
...
function QF(particles, idx, wf::RBM)
    X = particles.positions
...
function QF!(qf, particles, idx, wf::RBM)
    X = particles.positions
    for (i, m) in enumerate(idx)
        sum1 = 0.0
        for n in 1:wf.N
...
function paramDer(particles, wf::RBM)
    X = particles.positions
    da = 1 / wf.σ^2 * (X .- wf.a)
    sig = wf.b
    for m in 1:wf.M
...
function applyGradient(wf::RBM, grad)
    a = wf.a .- grad[1]
    b = wf.b .- grad[2]
    W = wf.W .- grad[3]
