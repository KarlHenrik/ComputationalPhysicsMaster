...
void imp2D(System &system, double dt, int t_steps) {
    ...
    int jobs = n - 2; // We give each thread a number of x values to calculate new_u for
    ...
    for (int t_i = 0; t_i < t_steps; t_i++) {
        for (int iter = 0; iter < maxIter; iter++) {
            diff = 0;
            # pragma omp parallel shared(new_U)
            {
                // Calculating new_U for the evenly distributed x values
                for (int job_x = 0; job_x < job_size; job_x++) {
                    x = omp_get_thread_num() * job_size + job_x + 1;
                    for (int y = 1; y < n - 1; y++) {
                        new_U(x, y) = 1.0 / (1 + 4 * alpha) * (alpha * ( U(x + 1, y) + U(x - 1, y) + U(x, y + 1) + U(x, y - 1)) + old_U(x, y) );
                        diff += fabs(U(x, y) - new_U(x, y));
                    }
                }
                // Calculating new_U for leftover values
                if (omp_get_thread_num() < extra_jobs) {
                    x = extra_jobs_start + omp_get_thread_num() + 1;
                    for (int y = 1; y < n - 1; y++) {
                        new_U(x, y) = 1.0 / (1 + 4 * alpha) * (alpha * ( U(x + 1, y) + U(x - 1, y) + U(x, y + 1) + U(x, y - 1)) + old_U(x, y) );
                        diff += fabs(U(x, y) - new_U(x, y));
                    }
...
/*
// NO PARALLELIZATION, for reference
void imp2D(System &system, double dt, int t_steps) {
    double alpha = dt / (system.dx * system.dx) * system.D;
    mat U = system.U;
    int n = U.n_rows;
